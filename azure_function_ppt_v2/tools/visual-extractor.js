...
/**
 * Visual Template Extractor
 * Creates a simple PowerPoint file that displays each master slide for visual analysis
 */
class VisualExtractor {
    ...
    async extractVisualLayouts() {
        console.log('[VisualExtractor] Creating visual analysis PowerPoint...');
        
        // Create a simple script that generates a test presentation using your template
        const testScript = this.generateTestScript();
        
        // Write the test script
        const scriptPath = path.join(__dirname, 'template-test.js');
        fs.writeFileSync(scriptPath, testScript);
        
        console.log(`[VisualExtractor] Test script created: ${scriptPath}`);
        console.log('Run this script to generate a visual analysis file:');
...
// Create and run the visual extractor
const templatePath = path.join(__dirname, '../template/ncs_ppt_template_2023.pptx');
const extractor = new VisualExtractor(templatePath);
extractor.extractVisualLayouts();